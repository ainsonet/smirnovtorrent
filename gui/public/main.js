...
async function addTorrent() {
  ...
  try {
    ...
    const downloadId = 'demo-' + Date.now();
    ...
    log(`Torrent added: ${downloadId}`, 'success');
    renderDownloads();
  ...
  updateInterval = setInterval(() => {
    downloads.forEach((dl, id) => {
      if (dl.status === 'downloading') {
        dl.progress = (dl.progress + 0.1).min(100.0);
        dl.downloaded = (dl.totalSize * dl.progress / 100.0);
        dl.downloadSpeed = 1024 * 1024;
        dl.uploadSpeed = 512 * 1024;
        dl.peers = 15;
      }
    });
    renderDownloads();
